...
* Improved large-site exports by resuming uploads and crawler discovery across background requests
* Preserved extensionless additional file paths during exports
* Added export discovery for custom post type archive URLs

= 3.8.10 =

* Fixed Rank Math redirect exports without relying on origin HTTP requests
* Fixed AIOSEO sitemap stylesheet exports and destination URL replacements
* Restored ZIP download URLs in plain-text debug logs
...
* Fixed ZIP download URLs when the WordPress uploads directory is outside wp-content
* Prevented exports from reporting success when the generated ZIP is missing or not publicly accessible
...
* Fixed exports so failed pages are skipped instead of cancelling the entire export
* Fixed numeric custom page pagination exports
* Excluded private backup uploads from exports

= 3.8.0 =

* Improved security and reliability across exports, integrations, and filesystem operations
* Improved background processing, retry handling, and large-site performance
* Improved admin accessibility, status handling, and automated regression coverage
* Updated compatibility requirement for Simply Static Pro 2.5.0
...
* Fix crawler user agent default value
* Update Studio-related environment UI
* Handle double-slash offline runtime paths
* Fix origin URL replacement for WP asset paths
* Avoid adding __qs directory if not needed

= 3.7.7 =

* Fix changes-only export scope
...
* Invert post type selection setting
* Exclude data permalinks from URL discovery
* Improve Elementor gallery asset exports

= 3.7.5 =

* Fixed preserve trailing slashes in Hide WP path mapping
* Fixed preserve referenced asset redirect pages
* Fixed local asset redirect handling (301)
...
* Fix cancelled background queue cleanup
...
* collect and remove trashed/draft posts from DB table
