...
namespace Simply_Static\Tests\Unit {
	...
	final class BootstrapCompatibilityTest extends UnitTestCase {
		...
		public function test_release_metadata_and_bootstrap_hook_are_aligned(): void {
			...
			self::assertMatchesRegularExpression( '/^ \* Version:\s+3\.8\.14$/m', $bootstrap );
			self::assertMatchesRegularExpression( '/^ \* Requires at least:\s+6\.2$/m', $bootstrap );
			self::assertStringContainsString( "define( 'SIMPLY_STATIC_VERSION', '3.8.14' );", $bootstrap );
			...
			self::assertMatchesRegularExpression( '/^Stable tag:\s+3\.8\.14$/m', $readme );
			...
			self::assertStringContainsString( '= 3.8.14 =', $readme );
			self::assertSame( '3.8.14', $package['version'] ?? null );
			self::assertSame( '3.8.14', $lock['version'] ?? null );
			self::assertSame( '3.8.14', $lock['packages']['']['version'] ?? null );
		}
