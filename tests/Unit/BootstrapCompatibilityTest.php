...
namespace Simply_Static\Tests\Unit {
	...
	final class BootstrapCompatibilityTest extends UnitTestCase {
		...
		public function test_release_metadata_and_bootstrap_hook_are_aligned(): void {
			...
			self::assertMatchesRegularExpression( '/^ \* Version:\s+3\.8\.12$/m', $bootstrap );
			self::assertMatchesRegularExpression( '/^ \* Requires at least:\s+6\.2$/m', $bootstrap );
			self::assertStringContainsString( "define( 'SIMPLY_STATIC_VERSION', '3.8.12' );", $bootstrap );
			...
			self::assertMatchesRegularExpression( '/^Stable tag:\s+3\.8\.12$/m', $readme );
			self::assertMatchesRegularExpression( '/^Tested up to:\s+7\.1$/m', $readme );
			self::assertMatchesRegularExpression( '/^Requires at least:\s+6\.2$/m', $readme );
			self::assertStringContainsString( '= 3.8.12 =', $readme );
			self::assertSame( '3.8.12', $package['version'] ?? null );
			self::assertSame( '3.8.12', $lock['version'] ?? null );
			self::assertSame( '3.8.12', $lock['packages']['']['version'] ?? null );
		}
