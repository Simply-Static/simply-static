...
namespace Simply_Static\Tests\Unit {
	...
	final class FileIntegrityPageHandler extends Page_Handler {
		...
		public function after_file_fetch( $destination_dir ) {
			...
			if ( null !== $this->generated_content ) {
				file_put_contents( $destination_dir . $this->page->file_path, $this->generated_content );
			}
		}
	}
	...
	final class FileIntegrityTest extends UnitTestCase {
		...
		public function test_zero_byte_http_response_is_removed_from_export(): void {
			...
			self::assertNull( $page->content_hash );
			self::assertSame( '2026-07-12 12:00:00', $page->last_modified_at );
			self::assertSame( 1, $page->get_handler()->after_fetch_calls );

			$task = new Fetch_Urls_Task();
			$task->archive_dir = trailingslashit( $this->archive_dir );
			$task->handle_200_response( $page, true, true );

			self::assertSame( 2, $page->save_calls );
		}

		public function test_content_created_by_after_fetch_handler_is_kept(): void {
			WpEnv::$remote_response = array(
				'response' => array( 'code' => 200 ),
				'headers'  => array( 'content-type' => 'text/html' ),
				'body'     => '',
			);
			$page = $this->pageFor( 'https://example.test/generated' );
			$page->get_handler()->generated_content = 'generated by handler';

			self::assertTrue( $this->fetcher->fetch( $page, false ) );

			$file_path = $this->archive_dir . '/generated/index.html';
			self::assertFileExists( $file_path );
			self::assertSame( 'generated by handler', file_get_contents( $file_path ) );
			self::assertSame( 'generated/index.html', $page->file_path );
			self::assertSame( 1, $page->get_handler()->after_fetch_calls );
		}
