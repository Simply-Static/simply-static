...
abstract class Crawler {
	 ...
	 * @return array {
	 *    @type bool $is_done Whether the crawler is done.
	 *    @type int $urls_added Number of URLs added in this batch.
	 *    @type int $new_offset New offset for next batch.
	 * }
	 */
	public function add_urls_to_queue( int $offset = 0 ): array {
		$urls       = $this->detect();
		$count      = 0;
		$batch_size = apply_filters( 'simply_static_crawler_batch_size', 100 );

		// Handle offset for non-streaming crawlers (that still return full array in detect)
		$urls_to_process = array_slice( $urls, $offset, $batch_size );
		$is_done         = ( $offset + count( $urls_to_process ) ) >= count( $urls );
		...
		return [
			'is_done'    => $is_done,
			'urls_added' => $count,
			'new_offset' => $offset + count( $urls_to_process ),
		];
	}
